...
#include "src/sounds/healing_fixed.wav.h"
...
// Note that the state-machine sets this back to false once it has completed playing a sound.
...
void sound_enableTxFifo() {
  Xil_Out32(AUDIO_CTRL_BASEADDR + I2S_CTRL_REG, 0b001);  //Enable TX Fifo, disable mute
}
...
void sound_sendDataToBothChannels(uint32_t sampleValue) {
  Xil_Out32(AUDIO_CTRL_BASEADDR + I2S_TX_FIFO_REG, sampleValue); // add to left Channel.
  Xil_Out32(AUDIO_CTRL_BASEADDR + I2S_TX_FIFO_REG, sampleValue); // add to right Channel.
}

// Used to set the volume. Use one of the provided values.
void sound_setVolume(sound_volume_t volume) {
  sound_currentVolume = volume;
}
...
//  debugStatePrint();
  ...
  case sound_wait_st:
    ...
    break;
  case sound_play_st:
    // Each time you enter this state, add as many samples as will fit in the FIFO.
    if (sound_array == NULL) {
      printf("ERROR, sound_tick: sound array has not been set.\n");
      return;
    }
    // This while-loop continues to load sound-data into the FIFOs until it is full or
    // the sound data are exhausted.
    while (!(Xil_In32(AUDIO_CTRL_BASEADDR + I2S_FIFO_STS_REG) & 0b0010)) { // while room in FIFO.
      uint32_t sampleValue = sound_array[arrayIndex] * sound_currentVolume;   // Scale by volume.
      sound_sendDataToBothChannels(sampleValue);  // Send the sound data to the left and right channels.
...
void sound_stopSound() {
  ...
  currentState = sound_wait_st; // Force the state-machine back to the wait state.
}
...
void sound_setSound(sound_sounds_t sound) {
  ...
  sound_array = NULL;  // Set the pointer to NULL so you can detect it never being set.
  switch(sound) {
  case sound_gameStart_e:
    sound_array = gameBoyStartup_wav;  // Set the array holding the data.
    sound_sampleCount = GAMEBOYSTARTUP_WAV_NUMBER_OF_SAMPLES;  // Size of the array.
    break;
  case sound_gunFire_e:
    sound_array = bcfire01_48k_wav;  // Set the array holding the data.
    sound_sampleCount = BCFIRE01_48K_WAV_NUMBER_OF_SAMPLES;  // Size of the array.
    break;
  case sound_hit_e:
  ...
  case sound_healing_e:
    sound_array = healing_fixed_wav;
    sound_sampleCount = HEALING_FIXED_WAV_NUMBER_OF_SAMPLES;
    break;
  case sound_healed_e:
...
/////////////////////////////////////////////////////////////////////
//// Simple test code.
// To run code, remove comments from ''void sound_runTest()'' to end of test code and
// make sure this is your only main() function.
//
// Note that the "interrupts_" functions are only necessary
// for the this stand alone test code.
//
// Make sure to leave this code commented-out when you are using the
// sound functions in your laser-tag game.
//
// The main functions you will use in the game are:
// - sound_init(): call this once prior to playing any sounds.
// - sound_playSound():
//     the sounds are provided in an enumerated type (sound_sounds_t) in the sound.h file.
// - You generally won't have to wait for ''sound_isSoundComplete() to complete,
//   this just keeps this test code from terminating before playing the sound in its entirety.
//
// Other useful functions include:
//   - sound_setVolume() provide an argument value from sound_volume_t, e.g.,
// sound_setVolume(sound_minimumVolume_e);
//
// Also, don't mess with the sample rate, the default sample rate is 48k
// and all of the sounds have been sampled at 48k.
//////////////////////////////////////////////////////////////////////////////
//void sound_runTest() {
//  printf("testing the sound...\n");
//  sound_init();                               // Init the sound hardware and related state machine.
...
//  sound_playSound(sound_gunFire_e);           // Start playing a sound.
//  while (!sound_isSoundComplete());           // Keep the program active until the sound has finished playing.
//  printf("exiting...\n");
//}
//
//int main() {
...
/**********************************************************************************
* Note from BLH:                                                                  *
* Most of this code was re-purposed from the original Digilent demonstration code. *
* The code initializes the IIC controller that is connected to the audio CODEC.   *
* It also provides functions to initialize the audio CODEC and to send/received   *
* to/from CODEC.
**********************************************************************************/
...
int AudioInitialize(u16 timerID,  u16 iicID, u32 i2sAddr)
{
  ...
  TimerInitialize(timerID); // This is a timer that is used to create precise delays.
  ...
  Status |= AudioRegSet(&Iic, 0, 0b000010111); // Left-channel ADC input volume.
  Status |= AudioRegSet(&Iic, 1, 0b000010111); // Right-channel ADC input volume.
  Status |= AudioRegSet(&Iic, 2, 0b101111001); // Left-channel DAC volume. Also set right volume to same value.
  Status |= AudioRegSet(&Iic, 4, 0b000010000); // Analog audio path.
  Status |= AudioRegSet(&Iic, 5, 0b000000000); // Digital audio path.
  Status |= AudioRegSet(&Iic, 7, 0b000001010); //Changed so Word length is 24  Status |= AudioRegSet(&Iic, 8, 0b000000000); //Changed so no CLKDIV2
  Status |= AudioRegSet(&Iic, 8, 0b000000000); //Changed so no CLKDIV2
  TimerDelay(75000);                           // Wait for things to settle down.
  Status |= AudioRegSet(&Iic, 9, 0b000000001); // Make things active.
  Status |= AudioRegSet(&Iic, 6, 0b000100000); // Power-up the ouput (OSC is left disabled as MCLK pin provides clock).
  ...
  // Not sure what the problem is, perhaps the DLL is not running at the correct frequency?
  // or, there is a bug in the IP that drives the CODEC.
  // In any case, the sampling rate is 48k.
  i2sClkDiv = 3;
